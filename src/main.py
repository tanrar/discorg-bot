...
async def update_last_summarized_message_id(channel_id, message_id):
    logger.info(f"Updating last summarized message ID for channel {channel_id} to {message_id}")
    table.put_item(Item={
...
async def fetch_messages_with_pagination(channel, last_message_id, max_messages=1000, time_limit=None):
    logger.info(f"Fetching messages for channel {channel.id}, starting from message {last_message_id}")
    messages = []
    last_id = last_message_id
    now = datetime.utcnow()
    
    while len(messages) < max_messages:
        new_messages = await channel.history(limit=100, after=discord.Object(id=last_id), oldest_first=False).flatten()
        if not new_messages:
            break
...
async def summarize_conversation(channel_id, command_message):
    ...
    messages = await fetch_messages_with_pagination(channel, last_summarized_id, max_messages=1000, time_limit=time_limit)
